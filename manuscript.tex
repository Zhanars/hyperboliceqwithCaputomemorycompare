...
\subsection{On the Applicability of Higher-Order Memory Schemes}\label{sec:discussion_l2sigma}

A natural question arises whether the $L2$-$1_\sigma$ scheme of Alikhanov~\cite{ref21}, which achieves $\mathcal{O}(\Delta t^{3-\alpha})$ accuracy for the Caputo derivative, could be employed in the present framework to improve the temporal convergence beyond $\mathcal{O}(\Delta t^{2-\alpha})$. We note that the $L2$-$1_\sigma$ formula has been successfully applied to fractional diffusion-wave equations of the form $\partial_t^\alpha u = \Delta u + f$ with $\alpha \in (1,2)$~\cite{ref21,sun2006}, where the \emph{leading} time derivative is itself fractional and all terms in the equation are naturally evaluated at the Alikhanov intermediate point $t_{n-1+\sigma}$ ($\sigma = 1-\alpha/2$).

The equation considered in the present work, however, belongs to a structurally different class: it is a standard second-order hyperbolic equation with a \emph{subordinate} fractional memory term ($\alpha \in (0,1)$),
\[
  u_{tt} - c^2\,u_{xx} + \gamma\,{}^C\!D_t^\alpha u = f.
\]
Here, the integer-order $u_{tt}$ is the principal temporal operator, discretized by the Newmark scheme ($\beta = 1/4$) at time level $t_n$. Replacing the $L1$ weights with the $L2$-$1_\sigma$ weights, which approximate the Caputo derivative at the shifted point $t_{n-1+\sigma} \neq t_n$, introduces a systematic $\mathcal{O}(\Delta t)$ mismatch between the evaluation points of the wave operator and the memory term. Our preliminary numerical tests confirm this: when $L2$-$1_\sigma$ weights are substituted into the Newmark framework while all other terms remain at $t_n$, the observed temporal convergence rate degrades to $\mathcal{O}(\Delta t)$ rather than improving; conversely, the standard $L1$ weights, which are consistent with the Newmark evaluation at $t_n$, reproduce the expected $\mathcal{O}(\Delta t^{2-\alpha})$ rate.

A fully $\sigma$-weighted scheme, in which \emph{all} operators (including $u_{tt}$) are simultaneously evaluated at $t_{n-1+\sigma}$ via a three-level interpolation, deserves separate investigation. The main technical challenge is that the $\sigma$-weighted second difference approximation
\[
  \sigma\,\delta_{tt}^n u + (1-\sigma)\,\delta_{tt}^{n-1} u \approx u_{tt}(t_{n-1+\sigma})
\]
requires a carefully designed initialization of $U^{-1}$ and a revised stability analysis that accounts for the full three-level energy structure. Whether such a reformulation can yield a provably second-order-in-time scheme for the hyperbolic-memory class, on par with the established $L2$-$1_\sigma$ results for parabolic-fractional models, remains an open problem that merits further research.
